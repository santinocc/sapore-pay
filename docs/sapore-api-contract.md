...
> What the public `sapore-pay` service and mini app may depend on from the private Sapore
> API, and what the private API adds for them. Copy this file into the public repo's
> `docs/` at kickoff so judges can see the integration surface without the private code.
...
|---|---|---|---|
| `POST /auth/login`, `POST /auth/signup`, `POST /auth/oauth` | — | mini app (fallback login) | Returns `{ token, user }`; token is RS256 (`kid` header), 30 days. |
| `GET /.well-known/jwks.json` | — | service, Privy | Public keys to verify Sapore access tokens (`iss` = `JWT_ISSUER`, default `sapore`). |
| `GET /recipes/explore?lat&lng&radiusKm…` | Bearer | mini app | Nearby published dishes for the swipe/list view. |
| `GET /recipes/:id` | Bearer | mini app | Dish detail (ingredients redacted until purchased). |
| `POST /payments/cart-quote` `{ recipeIds }` | Bearer | mini app, service | `{ requiredUsd, breakdown[] }` — the authoritative cart total (already excludes owned recipes). |
| `GET /payments/purchased-recipes`, `GET /payments/check-purchase/:recipeId` | Bearer | mini app | Cookbook / ownership checks after a payment lands. |
| `GET /users/me`, `GET /users/me/chef-profile` | Bearer | mini app | Roles, chef alias/avatar/country, `payoutTermsAccepted`, current payout wallets. |
| `PATCH /users/me/wallet` `{ ethWalletAddress \| solWalletAddress }` | Bearer | mini app | Enroll a payout address (48h lock rule applies). |
| `POST /users/me/payout-terms/accept` | Bearer | mini app | Consent gate every payout job checks. |

Legacy crypto flow (`POST /payments/crypto/register-sender`, `POST /payments/verify-crypto-cart`)
is what the service replaces; the mini app must not call it.

## To add in the private repo (thin glue; not judged, may be built before the event)

| Method & path | Auth | Purpose |
|---|---|---|
| `POST /auth/wallet-login` `{ message, signature }` (SIWE) | — | Mini app sign-in with the World App / embedded wallet. Creates or links the account by verified address and returns a normal Sapore token. |
...
```
PAY_SERVICE_URL=https://pay.sapore.food
