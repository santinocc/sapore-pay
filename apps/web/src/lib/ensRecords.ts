/**
 * ensRecords.ts — writing a Chef's payout and profile records once their
 * `<alias>.sapore.eth` resolver exists.
 *
 * Unlike ensClaim.ts's subname creation, this part is genuinely real, not
 * simulated-until-wired: ENSv2's "For App Developers" guide is explicit that
 * "the setter functions themselves are unchanged from ENSv1's public
 * resolver interface" — so `setAddr`/`setText`/`multicall` below are the
 * actual, long-stable ENS resolver ABI, not a guess. What's ENSv2-specific
 * is finding the RIGHT resolver (each account gets its own instance now,
 * there's no shared Public Resolver to assume) and who's authorized to write
 * to it (Enhanced Access Control roles) — both handled below, per the
 * guide's "Find the Resolver" / "Who Can Write" sections.
 *
...
import { evmCoinType } from '@sapore-pay/ens'
import {
  type Address,
  encodeFunctionData,
  type PublicClient,
...
/** Matches the app-wide TEMPO_CHAIN_ID assumption (see .env.example). */
